...
def calculate_ml(image):
    ...
    for x in range(1, dim-(2*q)):
        for y in range(1, dim-(2*q)):
            der_x = 0
            der_y = 0
            for i in range(3):
                for j in range(3):
            ...
            fxx[x,y] = der_x
            fyy[x,y] = der_y
            ML [x,y] = np.sum((np.abs(fxx[[x, y]]))+np.abs(fyy[[x, y]]))
    return ML     
...
q = 2
image = cv.imread('one_padded_image.jpg',cv.COLOR_BGR2GRAY)
sml = calculate_sml(image,2)
print(sml.shape)
...
for i in range(1,101):
    ...
    image = mat_content[frame_key]
    padded_image = pad(image,q)
    # print(stacked[:,:,i].shape)
    stacked[:,:,i] = calculate_sml(padded_image,2)
